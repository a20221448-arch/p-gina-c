...
if st.session_state.etapa == "seleccion_nivel":
    ...
    placeholder_nivel = "-- Selecciona un nivel --"
    opciones_nivel = [placeholder_nivel] + niveles_disponibles
    ...
    if nivel_elegido == placeholder_nivel:
        nivel_elegido = None
...
# ---------------------------------------------------------
    ...
    if not st.session_state.respondido:
        placeholder_resp = "SIN_RESPUESTA"
        claves_opciones = [placeholder_resp] + list(opciones.keys())

        etiqueta_elegida = st.radio(
            "Tu respuesta:",
            claves_opciones,
            format_func=lambda k: "-- Selecciona una opción --" if k == placeholder_resp else f"{k}) {opciones[k]}",
            key=f"respuesta_{st.session_state.indice}"
        )

        if etiqueta_elegida == placeholder_resp:
